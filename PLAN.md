...
- **레지스터 패널:** PC, 레지스터 파일, 메모리 쓰기. 이번 사이클에 바뀐 값을 강조한다. 값은 16진수·10진수·2진수를 한 줄에 함께 보여준다. 레지스터마다 주 표시 진법을 클릭 한 번으로 바꾼다. 10진수는 부호 있음·없음을 고르고, 2진수는 4비트씩 끊어 보인다. Hallym MIPS의 레지스터 창과 같은 모양이다(역할별 묶음, `$name`과 번호, 16진수와 10진수, 바뀐 값은 굵은 청록색). 트랙 A에서는 같은 표시를 하는 다중 진법 Probe 부품을 라이브러리에 넣는다.
...
- [ ] QtSpim의 분기 오프셋 인코딩이 교재 데이터패스(PC+4 기준)와 같은지, 지연 분기 설정별로 확인(0단계). 다르면 과제 표준 설정을 그에 맞춰 정한다
