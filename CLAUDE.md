...
- **PLAN.md 11장 편집기 개선 전체**를 구현한다. 전부 UI 층이다. 엔진과 .circ 안의 툴바·마우스 매핑·라벨 글꼴은 바꾸지 않고, 사용자 설정은 앱 환경설정에, 사용자가 직접 지정한 정보(터널 색, 신호 그룹, 영역 메모)만 PLAN.md 7.0 네임스페이스로 저장한다(11.0).
