...
| 44 | 새 파일의 Hallym MIPS(아직 파일에 없음 표시), 검색, 첫 부품 뒤 트리, 저장 뒤 jar 알림(V-01) |
| 45 | 같은 이름 파일 탭의 폴더 덧말과 Window 메뉴(V-05) |
| 46 | 빈 캔버스 안내, Help › Examples, 예제 읽기 전용 알림(V-07) |
| 47 | ref-mips 상태 표시줄 PC, Tunnels 외톨이 터널, Register 메뉴 Mark as PC(V-08) |
| 48 | 환경설정 없는 첫 실행: 새 창이 작업 영역을 채운다(X-01). 로그에 창·작업 영역 크기 |
| 49 | 도구 모음 넘침: 960 폭(아이콘만)·640 폭(» 메뉴, 연 상태)(X-02) |
| 50 | 좁은 창의 칸 비율: 1280·960 폭에서 캔버스가 절반 이상(X-03) |
...
장면마다 앱 상태를 기준으로 되돌리고 검사한다(V-09, D-104): 열린 탭·표시 모드·선택·배율·스크롤·편집. 어긋나면 촬영이 실패한다. 장면 31은 여전히 따로 찍는다(28이 프로그램을 올린 상태가 되돌리기 범위 밖일 수 있음).
