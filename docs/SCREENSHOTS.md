...
- **참조 회로:** `tests/mips/ref-mips.circ`는 서브회로가 없다. 서브회로 상자 표시는 `tests/circ/subcircuit.circ`로 찍는다.
...
| 02 | 참조 MIPS 회로 전체 100%, 같은 장면 원조 2.7.1 |
| 03 | 같은 회로 200% 부분: 라벨 칩, 터널 색, 버스 이름, 스플리터 팔 라벨(원조 비교). 서브회로 포트 이름·회로 이름 캡션 |
...
| 10 | 파일 탭 3개 이상 |
| 11 | .s 불러오기 요약, 명령어 메모리·데이터 메모리·스택·콘솔 부품(재귀 factorial 실행 중과 끝) |
| 12 | 마우스 오버 정보(부품, 포트) |
| 13 | ? 단축키 표 |
